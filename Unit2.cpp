...
TShmigirev_UVP13_EDT *Shmigirev_UVP13_EDT;
struct event_type{
  char udk[10];
  char name[50];
  char type_evt[30];
...
//---------------------------------------------------------------------------
__fastcall TShmigirev_UVP13_EDT::TShmigirev_UVP13_EDT(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TShmigirev_UVP13_EDT::btOkClick(TObject *Sender)
{
...
void create_file(){
  ...
  strcpy(event.udk, Shmigirev_UVP13_EDT->edtUDK->Text.c_str());
  strcpy(event.name,Shmigirev_UVP13_EDT->edtName->Text.c_str());
  strcpy(event.type_evt,Shmigirev_UVP13_EDT->cmbTyptEvt->Text.c_str());
  strcpy(event.date_evt,Shmigirev_UVP13_EDT->dtpDateEvt->Date.DateString().c_str());
  strcpy(event.people_cnt,Shmigirev_UVP13_EDT->edtPeopleCnt->Text.c_str());
  strcpy(event.fio_heroes,Shmigirev_UVP13_EDT->edtFioHeroes->Text.c_str());
  fwrite(&event,sizeof(event),1,p);
  fclose(p);
  Kazachenko_UVP3->Show();
  Shmigirev_UVP13_EDT->Close();
}
...
void TShmigirev_UVP13_EDT::show_result()
{
  FILE *p;
...
void insert_by_number()
{
  int i,n;
  FILE *p1,*p2;
  n = StrToInt(Shmigirev_UVP13_EDT->edtNumRec->Text);
...
{
  char type_evt[30];
  strcpy(type_evt, Shmigirev_UVP13_EDT->cmbTyptEvt->Text.c_str());
  for (int i =1; i < Kazachenko_UVP3->sgFindResult->ColCount; i++)
    for (int j=1; j < Kazachenko_UVP3->sgFindResult->RowCount; j++)
      Kazachenko_UVP3->sgFindResult->Cells[i][j] = "";
  FILE *p;
...
void delete_by_name(){
    ...
    char lv_name[50];
    int lv_kol = 0;
    strcpy(lv_name,Shmigirev_UVP13_EDT->edtName->Text.c_str());
    if ( ( p1 = fopen( Kazachenko_UVP3->_fileName, "rb" ) ) == NULL ){
      MessageDlg("Ошибка чтения исходного файла",mtWarning,TMsgDlgButtons()<<mbOK,0);
      return;
    };
...
void change_count(){
    ...
    strcpy(lv_name, Shmigirev_UVP13_EDT->edtName->Text.c_str() );
    strcpy(lv_people_cnt, Shmigirev_UVP13_EDT->edtPeopleCnt->Text.c_str() );
    if ( ( p1 = fopen( Kazachenko_UVP3->_fileName, "rb" ) ) == NULL ){
      MessageDlg("Ошибка чтения исходного файла",mtWarning,TMsgDlgButtons()<<mbOK,0);
      return;
    };
...
void __fastcall TShmigirev_UVP13_EDT::FormShow(TObject *Sender)
{
    switch (type_operation) {
    case 1:
      Shmigirev_UVP13_EDT->Caption = "Создание нового файла";
      edtUDK->Visible = true;
      ...
      break;
    case 2:
      Shmigirev_UVP13_EDT->Caption = "Добавление записи в конец файла";
      edtUDK->Visible = true;
      edtName->Visible = true;
      edtNumRec->Visible = false;
      edtPeopleCnt->Visible = true;
      ...
      break;
    case 3:
      Shmigirev_UVP13_EDT->Caption = "Добавление записи по номеру";
      edtUDK->Visible = true;
      edtName->Visible = true;
      ...
      break;
    case 4:
      Shmigirev_UVP13_EDT->Caption = "Удаление записи в конец файла";
      ...
      break;
    case 5:
      Shmigirev_UVP13_EDT->Caption = "Показать записи по типу";
      edtUDK->Visible = false;
      edtName->Visible = false;
      edtNumRec->Visible = false;
      edtPeopleCnt->Visible = false;
      ...
      break;
    case 9:
      Shmigirev_UVP13_EDT->Caption = "Удалить записи по заданному имени";
      edtUDK->Visible = false;
      edtName->Visible = true;
      edtNumRec->Visible = false;
      edtPeopleCnt->Visible = false;
      ...
      break;
    case 10:
      Shmigirev_UVP13_EDT->Caption = "Изменить количество для записей с заданным именем";
      edtUDK->Visible = false;
      edtName->Visible = true;
      edtNumRec->Visible = false;
      edtPeopleCnt->Visible = true;
...
void __fastcall TShmigirev_UVP13_EDT::btCancelClick(TObject *Sender)
{
   Kazachenko_UVP3->Show();
   Shmigirev_UVP13_EDT->Close();
}
